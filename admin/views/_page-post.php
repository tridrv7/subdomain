        ...
        <script>
            ...
            $(document).ready(function(){
                ...
                $('#_table').dataTable(
                {
                    processing: true,
                    serverSide: true,
                    ajax: 'controllers/_ppage-post.php?_act=5&ka_id=<?=$_REQUEST['ka']?>',
					order: [[2, 'desc']],
                    columnDefs: [ 
						{
							targets: 0,
                ...
                $('#btnSimpan').click(function(){
                    ...
                    else if ($('#post_desk').val().search("'")!==-1){
						$('#post_desk').val($('#post_desk').val().replace("'", "`"));
                        // focus ke input nama_barang
                        $( "#post_desk" ).focus();
                        // tampilkan peringatan data tidak boleh kosong
                        toastr['warning']('Terdapat karakter yang tidak diijinkan<br>karakter akan diganti secara otomatis');
                    } 
                    ...
                    // jika semua data sudah terisi, jalankan perintah simpan data
                        ...
                        $.ajax({
                            type : "POST",
                            url  : "controllers/_ppage-post.php",
                            data : data,
                            contentType: false,
                            processData: false,
                ...
                $('#_table tbody').on( 'click', '.getUbah', function (){
					...
					$.ajax({
						type : "GET",
						url  : "controllers/_ppage-post.php?_act=4",
