    ...
    <div class="panel-container show">
                        ...
                        <div class="modal-body">
                          ...
                          <div id="_mAttr">
							...
							<div class="form-group">
								<label for="example-input-small" class="form-label">Nama <?=$txtName?></label>
								<input type="text" id="jab_nm" name="jab_nm" class="form-control form-control-sm text-capitalize">
							</div>							
							<div class="form-group">
								<textarea class="form-control" id="jab_desk" name="jab_desk" placeholder="Post Content" rows="25"></textarea>
							</div>							
                        </div>
        ...
        $(document).ready(function(){
            ...
            $('#_table').dataTable(
            {
                processing: true,
                serverSide: true,
                ajax: 'controllers/_pmast-JabDept.php?_act=5&ka_id=<?=$_REQUEST['ka']?>',
                columnDefs: [ 
                    {
                        targets: 0,
                        visible: false,
            ...
            var table = $('#_table').DataTable( {
                ...
                "processing": true,
                "serverSide": true,
                "ajax": 'controllers/_pmast-JabDept.php?_act=5',     // panggil file data_transaksi.php untuk menampilkan data transaksi dari database
                "columnDefs": [ 
            ...
            */
            ...
            $('.btnTambah').click(function(reload){
				...
				$('#_mAttr').html('<input type="hidden" class="form-control" id="ka_id" name="ka_id" value="<?=$_REQUEST['ka']?>" autocomplete="off">');
				
				$('#jab_desk').summernote('reset');
            });
            ...
            $('#btnSimpan').click(function(){
                ...
                if ($('#jab_nm').val()==""){
					...
					toastr['warning']('Nama <?=$txtName?> tidak boleh kosong');
                } 
                // jika semua data sudah terisi, jalankan perintah simpan data
                    ...
                    $.ajax({
                        type : "POST",
                        url  : "controllers/_pmast-JabDept.php?_act="+_act,
                        data : data,
            ...
            $('#_table tbody').on( 'click', '.getUbah', function (){
				var jab_id = $(this).attr('id');

				$('#_mTitle').html('Ubah Data Sub Kategori');
				$('#_mAttr').html('<input type="hidden" class="form-control" id="jab_id" name="jab_id" autocomplete="off" value="'+jab_id+'"><input type="hidden" class="form-control" id="ka_id" name="ka_id" value="<?=$_REQUEST['ka']?>" autocomplete="off">');
				$("#_mForm").attr("_act","2");

				$.ajax({
                    type : "GET",
                    url  : "controllers/_pmast-JabDept.php?_act=4",
                    data : {jab_id:jab_id,ka_id:<?=$_REQUEST['ka']?>},
                    dataType : "JSON",
                    success: function(result){
                        ...
                        // tampilkan data transaksi
                        $('#jab_id').val(result.jab_id);
                        $('#jab_nm').val(result.jab_nm);
						$('#jab_desk').summernote('code', result.jab_desk);
                    }
                });
            });
            ...
            $('#_table tbody').on( 'click', '.getHapus', function (){
				...
				var vConfirm = '<form id="_mForm" enctype="multipart/form-data">'+
									'<input type="hidden" id="jab_id" name="jab_id" value="'+varIDGet+'">'+
			...
			$('#_mBtnConfirm').click(function(){
				...
				$.ajax({
					type : "POST",
					url  : "controllers/_pmast-JabDept.php",
					data : data,
					contentType: false,
					processData: false,
					success: function(result){     
